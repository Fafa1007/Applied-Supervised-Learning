...
f1_t_3 <- ggplot() + geom_line(data = df, aes(x = x, y = y)) + geom_vline(xintercept = mean(max_F1_tau), color = "red", linetype = "dashed") + theme_minimal() + labs(x = "Tau", y = "F1 Score", title = "F1 Score Against Different Desician Rule Thresholds (Tau) for a KNN Model")
f1_t_3
...
yhat_knn <- ifelse(knn_predict_probabilities >= max_F1_tau,'Revenue', "No Revenue")
y_knn <- matrix(ifelse(as.numeric(unlist(valid[,16]))==2, 'Revenue', "No Revenue"))
cat("Confusion Matrix using Predicted Probabilities")
(confmat <- table(yhat_knn, y_knn, dnn = c('Predicted label', 'True label')))
...
text(
  ...
  labels = paste("Threshold:", round(mean(max_F1_tau), 3)),  # Example metric
  col = "black"
)
